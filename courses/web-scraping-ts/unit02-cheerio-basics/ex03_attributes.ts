...
export function collectNavHrefs(html: string): string[] {
  ...
  $(".nav-link").each((index,element) => {result.push($(element).attr("href") ?? "")});
  return result;
}
...
export function collectSnsPlatforms(html: string): string[] {
  ...
  const result:string[] = [];
  const $ =cheerio.load(html);
  $(".sns-link").each((_,element) => {result.push($(element).attr("data-platform")?? "unknown")});
  return result;
}
...
export function firstAttr(html: string, selector: string, attrName: string): string | null {
  ...
  const $ = cheerio.load(html);
  return $(selector).first().attr(attrName) ?? null;
}
