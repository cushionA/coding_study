...
export function computeBackoffMs(
  ...
  // TODO: baseMs * 2^attempt にジッタ(0以上baseMs未満)を足した値を返す
...
export async function withRetry<T>(
  ...
  // TODO: fnを実行し、失敗したらcomputeBackoffMsで待ち時間を求めてsleepFnで待ち、
  // maxRetries回まで再試行する。最終的に全部失敗したら最後の例外を投げ直す
  throw new Error("TODO: 未実装");
}
...
export function cleanOneRecord(raw: RawRecord, parsers: FieldParsers): CleanRecord {
  // TODO: parsersを使ってname/price/stock/dateを変換し、
  // name===""、Number.isNaN(price)、Number.isNaN(stock)のいずれかならErrorを投げる
  throw new Error("TODO: 未実装");
}
...
export function cleanAllRecords(raws: RawRecord[], parsers: FieldParsers): CleanRecord[] {
  // TODO: 各rawをcleanOneRecordに通し、成功したものだけをresultsに集める。
  // 失敗(例外)した行はconsole.warnで警告してスキップする
  throw new Error("TODO: 未実装");
}
