...
def add_group_relative(df: pd.DataFrame, group_col: str, value_col: str) -> pd.DataFrame:
    # TODO: transformで元と同じ長さの平均を作り、2列追加したコピーを返す
    raise NotImplementedError


# 日時から曜日と曜日のsin/cos周期特徴を追加する
def add_weekday_cycle(df: pd.DataFrame, date_col: str) -> pd.DataFrame:
...
def add_lag_one(
    df: pd.DataFrame, entity_col: str, date_col: str, value_col: str
) -> pd.DataFrame:
