...
def split_holdout(
    ...
    shuffle = df.sample(frac=1,random_state=random_state)
    valid_count = int(len(shuffle) * valid_frac)
    return [shuffle.iloc[valid_count:],shuffle.iloc[:valid_count]]
...
def category_median_baseline_score(
    ...
    cat_median = {}
    for col in train_part[category_col].unique():
        cat_median[col] = train_part.loc[train_part[category_col] == col,target_col].median()
    median = train_part[target_col].median()
    pred = valid_part[target_col]
    pred = valid_part[category_col].map(cat_median)
    unseen = pred.isna().sum()
    pred = pred.fillna(median)
    return {"rmsle": rmsle(valid_part[target_col],pred), "n_unseen_category": unseen}
