...
このコースの演習では、公開関数の引数と戻り値に Python の型注釈を付ける。エディタの補完機能が使える場合は、`df: pd.DataFrame`、`image: np.ndarray`、`model: nn.Module` のような注釈からメソッド候補を出せる。

これは C# の宣言型に近い「この値には何が入るか」のメモであり、実行時に値を強制変換したり、テストの採点条件を変えたりはしない。`Any` / `object` は外部ライブラリの多態的なオブジェクトにだけ限定し、データフレーム・配列・Tensor・文字列・数値・コレクションには具体的な基本型を優先する。
...
### 実行環境の使い分け(開発 = ThinkPad / 学習 = クラウドGPU / 推論 = CPU)

この構成を前提に教材を書いてある。3つの役割を分けて考えるのが要点。

| 役割 | 場所 | 何をするか |
|---|---|---|
| **開発** | ThinkPad(CPU) | コードを書く、小データで正しさを確認する、教材の演習を解く |
| **学習** | クラウドGPU(時間借り) | 本番データで実際にモデルを学習する。月に数回・数時間 |
| **推論** | CPU(常時稼働) | 学習済みモデルで日々のデータを捌く |

このために、**コードは device 非依存に書く**。`device = torch.device("cuda" if torch.cuda.is_available() else "cpu")` を1箇所だけで定義し、モデルとバッチを `.to(device)` する。こうすると**同じノートブック・同じスクリプトが ThinkPad でもクラウドGPUでも無改変で動く**。教材の演習は常に CPU・極小データで採点するが、書き方は本番と同じにしてある。

**GPU 課金を無駄にしないための型**を Day 7 で仕込む。いきなり GPU で本番データを回さない:

1. ローカルCPUで **1バッチだけ**流して、shape と loss が出るか確認する
2. **数ステップ**回して loss が下がるか確認する
3. ここまで通ってから、本番データ・本番エポックで GPU へ持っていく

バグは 1・2 でほぼ出る。GPU を借りてから気づくと、その時間はまるごと無駄になる。

関連して扱うこと — VRAM の勘定(Day 7: バッチ × 系列長 × hidden × 層数。attention は系列長に**二乗**で効くので、メモリ不足のときは「バッチを半分」より「`max_length` を見直す」方が効く場面がある)、勾配累積とAMP(Day 7)、`num_workers` / `pin_memory` と「GPUは速いのにスループットが出ない = データ供給が律速」の診断(Day 9)、GPUで学習した重みをCPUで読む `map_location="cpu"`(Day 10)。

> ThinkPad は機種によって dGPU が無い、あっても VRAM 4〜8GB のことが多い。**ローカルで本気の学習を回すことは想定していない**。ローカルは「正しさの確認」まで、という割り切りが結果的に一番安く速い。

#### Day 9 で扱う転移学習について(教材の数値は実測済み)

この教材の検証環境からは事前学習済み重みをダウンロードできなかったため、**「別の合成データで backbone を先に学習させ、それを転用する」形で転移学習の効果を実測**してある(再現スクリプト: `unit08-image-and-pattern-basics/data/verify_transfer.py`)。
...
| スクラッチCNN(BN + 拡張 + cosine) | 0.9038 |
| 転移学習(全層ファインチューニング) | 0.9451 |
| **転移学習(backbone凍結 + 線形ヘッド)** | **0.9538** |

ここから3つのことが言える。**学習の作り込みだけで 0.57 → 0.90 と変わる**(アーキテクチャより先にレシピを疑う)、**作り込んだスクラッチに対しても転移学習は +0.05**、そして **小データでは凍結が全層ファインチューニングに勝つ**(0.9538 > 0.9451)。3つ目は「まず凍結して線形ヘッドだけ学習し、足りなければ段階的に解凍する」という実務の手順の根拠になる。
