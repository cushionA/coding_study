...
def augment_image(
    image: np.ndarray, horizontal_flip: bool = False, brightness: float = 1.0
) -> np.ndarray:
...
def stratified_group_splits(
    labels: np.ndarray, groups: np.ndarray, n_splits: int = 3, random_state: int = 42
...
def make_oof_predictions(
    ...
    labels: np.ndarray,
    groups: np.ndarray,
    feature_function: Callable[[np.ndarray], np.ndarray],
