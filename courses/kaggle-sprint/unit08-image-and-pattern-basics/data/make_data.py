...
train 801枚 / test 248枚。同一商品の写真は GroupKFold(product_key) で分けて評価する
(そうしないと unit02 で学んだグループリークをそのまま踏む)。
...
【分類ベースライン】
  ...
  → 色だけでは 0.35 しか出ない(5クラスなのでチャンスは 0.20)。
    形の情報を捉える HOG で 0.78 まで上がる。「何を特徴量にするか」が効くことの実証。

【★重複画像の検出 —— unit08 の目玉】
    同一商品ペア 378件 のうち、**ピクセル完全一致はわずか 3件**。
    つまりバイト比較・ハッシュの完全一致では 99% を取りこぼす。

    16x16 にリサイズ → グレースケール → 平均0分散1に正規化 → L2正規化したベクトルの
    コサイン類似度を使うと:
  ...
  → 閾値を下げると取りこぼしは減るが誤検出が増える、という
    precision/recall のトレードオフがそのまま表になる。
    名寄せ(unit06)で扱った閾値の決め方が、画像でも同じ形で出てくることを示せる。

【★転移学習の検証(unit09)—— オフラインで実測済み】

外部の事前学習済み重みは組織のエグレスポリシーで取得できない
(download.pytorch.org / huggingface.co ともに 403)。そこで「事前学習」を自前で作って検証した:

  unit08 とは別の8クラス(triangle/star/cross/ring/diamond/arrow/hexagon/bar)の
  合成画像3000枚を同じ描画パイプラインで生成 → その8クラス分類で CNN backbone を学習
  (= ImageNet の代役)→ その backbone を unit08 の5クラスタスクに転用。
  比較対象のスクラッチ版は**同じ構造・同じエポック数**で重みだけランダム初期化。
  ...
  【教訓1: 学習の作り込みが効く】
    同じデータ・同じ規模でも、BatchNorm・データ拡張・学習率スケジュールの有無だけで
    0.5731 → 0.9038 と変わる。「CNN が古典特徴に負けた」と結論する前に、
    まず学習の作り込みを疑うこと。アーキテクチャより先にレシピを見る。

  【教訓2: 転移学習は効く】
    十分に作り込んだスクラッチ CNN(0.9038)に対しても、事前学習した特徴を借りると
    +0.050(0.9538)。初期状態の重みだけが違う条件での比較なので、差の原因は明快。

  【★教訓3: 小データでは「凍結」が「全層ファインチューニング」に勝つ】
    凍結 0.9538 > 全層FT 0.9451。学習データが約600枚しかないため、
    全層を動かすと過学習する。**まず凍結して線形ヘッドだけ学習し、
    それで足りなければ段階的に解凍する**、という実務の手順の根拠がここにある。

  注: 上の数値は「自前で作った事前学習」によるもの。実際の ImageNet 事前学習モデル
  (torchvision / timm)を使えば、この題材でも同様かそれ以上の差が出ると期待される。
  演習はオフライン制約のためランダム初期化でロジックを採点するが、
  lesson では実モデルの使い方も解説する(ダウンロードを伴うセルはオプション扱い)。
"""
