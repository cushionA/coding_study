...
--------------------------------------------------------------------------
lesson.ipynb が実際に出力する数値(正規化の実装が上表と少し違うため差がある)
--------------------------------------------------------------------------

    前処理                        語彙数   accuracy   macro-F1
    ① 素のテキスト                4185     0.9243     0.9219
    ② 正規化してから               1524     0.9395     0.9384
    ③ 数字を全部潰す(やりすぎ版)   1530     0.9390     0.9381

  ①②の傾向は上表と同じ(語彙が激減して精度が上がる)。語彙数が 1857 ではなく
  1524 なのは正規表現の細部が違うだけで、**教材としてはこちらの方が効果が強い**。
  **演習を書くときは lesson.ipynb の実装に合わせること**(lesson が正本)。

  ★③の予想外の結果: 「数字を全部 <num> に潰せば語彙はさらに減る」と予想されるが、
  実測では **逆に増えた**(1524 → 1530)。`XR-500B` が `xr- <num> b` と3トークンに
  割れ、新しいトークンと bigram が生まれるため。
  「置換の結果が次工程(分かち書き)でどう切られるかまで見ないと、正規化の是非は
  判断できない」という教材になっており、lesson でそう扱っている。
"""
