...
export function mapFieldType(type: FieldType): BigQueryFieldType {
  // TODO: type に対応する BigQuery の型名を返す
  // (string→STRING, int→INT64, float→FLOAT64, boolean→BOOL, timestamp→TIMESTAMP)
...
export function buildSchemaField(spec: FieldSpec): SchemaField {
  // TODO: mapFieldType(spec.type) で type を求める。
  // spec.nullable が true なら mode は "NULLABLE"、それ以外(未指定含む)は
  // "REQUIRED" にして { name: spec.name, type, mode } を返す
  throw new Error("TODO: 未実装");
}

// 複数列をまとめて、テーブル全体のスキーマ(SchemaField[])に変換する。
export function buildTableSchema(specs: FieldSpec[]): SchemaField[] {
  // TODO: buildSchemaField を使って specs の各要素を変換した配列を返す
  // (ループを自分で書く必要はない)
  throw new Error("TODO: 未実装");
}
...
export function buildAuthorSummarySchema(): SchemaField[] {
  // TODO: 上記3列に対応する FieldSpec の配列を作り、buildTableSchema に渡して返す
  throw new Error("TODO: 未実装");
}
