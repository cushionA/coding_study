...
// 一覧を返すハンドラ。
// TODO: books配列全体を res.json(...) でそのまま返す(ステータスコードは既定の200でよい)
export function listBooksHandler(req: Request, res: Response): void {
  throw new Error("TODO: 未実装");
}

// 1件を返すハンドラ。
// TODO: req.params.id は文字列なので Number(...) で数値に変換し、findBookById で探す。
// 見つかれば res.json(book) を、見つからなければ
// res.status(404).json({ error: "book not found" }) を返す
