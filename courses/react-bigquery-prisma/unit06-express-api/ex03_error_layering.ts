...
// TODO:
// - err が ValidationError のインスタンスなら: res.status(400).json({ error: err.message })
// - err が NotFoundError のインスタンスなら: res.status(404).json({ error: err.message })
// - それ以外の想定外エラーなら: err.message や err.stack を絶対にレスポンスへ含めず、
//   console.error(err) でログにだけ出し、res.status(500).json({ error: "internal server error" })
