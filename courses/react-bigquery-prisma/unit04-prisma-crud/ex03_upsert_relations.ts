...
import type { PrismaClient, Book, Tag } from "./prisma/generated/prisma/client.js";
...
export async function upsertBookByIsbn(prisma: PrismaClient, input: UpsertBookInput): Promise<Book> {
  // TODO: prisma.book.upsert を呼ぶ。
  //  - where は isbn で1件を特定する形
  //  - update には「既存行を書き換える内容」(isbnは書き換えなくてよい)
  //  - create には「新規作成する場合の全フィールド」(isbnも含める)
...
export async function addTagToBook(
  ...
  // TODO: prisma.book.update を呼ぶ。
  //  - where で対象の本を1件特定する
  //  - data.tags に { create: { label } } を渡すと、更新と同時に
  //    そのTagがbookIdを自動で埋めた状態で作られる(ネストしたcreate)
  //  - include: { tags: true } を付けると、戻り値に更新後のタグ一覧が含まれる
  throw new Error("TODO: 未実装");
}
...
export async function getBookWithTags(prisma: PrismaClient, bookId: number): Promise<BookWithTags | null> {
  // TODO: prisma.book.findUnique を、where と include を組み合わせて呼ぶ
  throw new Error("TODO: 未実装");
}
