...
def make_scores(n_students: int, n_subjects: int, seed: int = 0) -> np.ndarray:
...
def student_averages(scores: np.ndarray) -> np.ndarray:
...
def count_passing(scores: np.ndarray, threshold: float) -> np.integer:
...
def curve_by_subject(scores: np.ndarray) -> np.ndarray:
