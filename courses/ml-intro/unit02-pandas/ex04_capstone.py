...
def mean_by_species(df: pd.DataFrame, column: str) -> pd.Series:
...
def filter_and_select(df: pd.DataFrame, filter_column: str, threshold: float, columns: list[str]) -> pd.DataFrame:
...
def inject_missing(df: pd.DataFrame, column: str, frac: float, seed: int = 0) -> pd.DataFrame:
    import numpy as np
...
def clean_and_summarize(df: pd.DataFrame, column: str, frac: float = 0.1, seed: int = 0) -> pd.DataFrame:
