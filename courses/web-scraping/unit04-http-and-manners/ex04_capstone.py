...
def get_body_with_retry(response, fetch_func, sleep_func, default_wait=1.0):
    # TODO: response.status_codeで分岐する。
    # 429ならRetry-Afterの秒数だけsleep_funcを呼んでからfetch_func()で1回だけ再取得しtextを返す。
    # ok(2xx)ならそのままtextを返す。それ以外はNoneを返す。
...
def fetch_if_allowed(rp, user_agent, path, fetch_func, sleep_func):
    # TODO: is_allowed で許可を確認してからfetch_func()を呼び、get_body_with_retryで本文を得る
    raise NotImplementedError
