...
### 2. パーソナライズ材料の収集
- `progress/*.json` の `skills` を読む: level>=3 の概念は新コースでは復習扱い(lesson で圧縮、演習は1つに)。level 1〜2 で止まっている概念は厚めに。
- `notes/` の直近数件から、つまずき傾向・有効だった説明スタイルを拾う。
- 初回(進捗なし)はプロファイルのみで設計。
...
### 5. カリキュラム設計
`curriculum-architect` を起動し、course.json(units[].concepts 必須)とコースREADMEを生成させる。パーソナライズ材料(手順2の要約)をプロンプトに含める。
結果(ユニット一覧・概念・マイルストーン)を**ユーザーに提示して承認を得る**。
