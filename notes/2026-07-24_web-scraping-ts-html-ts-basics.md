...
- `tel:` は電話アプリ・通話機能へ渡す。
- `#contact` のようなフラグメントは同じページ内の該当位置へ移動し、通常は通信しない。
- 独自スキームも対応アプリへ渡せる。URLはWebページの住所だけでなく、処理を担当するアプリへの指示にもなる。
- スクレイパーで巡回するURLは、`http:` / `https:` に絞る。`mailto:` や `tel:` は収集対象になっても通常は取得しない。
